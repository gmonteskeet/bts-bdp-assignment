...
Gold:   PostgreSQL tables (aircraft, observations)
"""
...
# from airflow.operators.python import PythonOperator # old
...
SCRAPING_URL = "https://samples.adsbexchange.com/index.html#readsb-hist/2023/11/01/"
FUEL_RATES_URL = "https://raw.githubusercontent.com/martsec/flight_co2_analysis/refs/heads/main/data/aircraft_type_fuel_consumption_rates.json"
# Only download a sample of files to keep it manageable (every 5 minutes = 60 files/hour)
# Each file is a 5-second snapshot. We'll grab one per minute for a few hours.
MAX_FILES = 200
...
# ---------------------------------------------------------------------------
# Task 1: Discover available JSON files
# ---------------------------------------------------------------------------
...
def bronze_to_silver(**context):
    ...
    all_records = []
    with ThreadPoolExecutor(max_workers=10) as pool:
        futures = {pool.submit(parse_one, f): f for f in files}
        for i, future in enumerate(as_completed(futures), 1):
            try:
